# Working document by HK, July 13 2022
...
TrainSuperMDSOnce <- function(d=NULL, y, alpha=.5, S=2, x=NULL, z=NULL){
  ...
  if(is.null(z))  z <- cmdscale(d, k=S) # classical MDS by default
  ...
  # z: nxS matrix of the configuration points obtained
  # crits: values of the criterion obtained at the iterations of the algorithm.
  # stress: portion of the final criterion value that are due to the STRESS component of the objective function
  # super: portion of the final criterion value that are due to the SUPERVISED component of the objective function
  while(length(crits)<2 || abs(crits[length(crits)]-crits[length(crits)-n])/min(crits) > 1e-2){
    # end the loop when the crits no longer changes (threshold 1%)
    for(i in 1:n){
      if(S>1){
        if(y[i]==1){
          znew <- (1-alpha)*apply(z[-i,],2,sum) 
            - (1-alpha)*apply(
              # sweep: return an array obtained from an input array by sweeping out a summary statistic  
              # by doing scale() it computes z[-i,] - z[i,], namely
              sweep(scale(z[-i,], center=z[i,], scale=FALSE), # matrix
                    1, # MARGIN (by row)
                    d[i,-i]/distmat(z)[i,-i], # STATS..
                    "*" # operation
                    # Multiplies STATS to every column
                    ), 
              2, sum
              )
            + alpha*apply(z[y==2,], 2, sum) 
            - (alpha/sqrt(S))*sum(d[i,y==2])  
