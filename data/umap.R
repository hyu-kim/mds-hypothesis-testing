...
y <- as.matrix(read.csv(sprintf('result/Evaluation/sim_1-Y.csv')))

for(k in c(5,10,20,30)){
  res <- umap(dist_mat, n_neighbors = k, y = factor(y))
  write.csv(res, sprintf('result/Evaluation/sim_1-umap_s-%02d-Z.csv',k), row.names=FALSE)
  res <- umap(dist_mat, n_neighbors = k)
  write.csv(res, sprintf('result/Evaluation/sim_1-umap_u-%02d-Z.csv',k), row.names=FALSE)
}
...
for(k in c(5,7,10)){
  ...
  write.csv(res, sprintf('result/Evaluation/alga_2-umap_s-%02d-Z.csv',k), row.names=FALSE)
  res <- umap(data_dist, n_neighbors = k)
  write.csv(res, sprintf('result/Evaluation/alga_2-umap_u-%02d-Z.csv',k), row.names=FALSE)
}
