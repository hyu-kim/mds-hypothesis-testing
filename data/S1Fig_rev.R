...
for(N in c(50, 100, 200, 500)){
  for(l in c((0:4)/50, (2:20)/20)){
    log_df <- read.csv(sprintf('result/ScalingStudy/sim_rev_1-N%d-fmds-%.2f-log.csv', N, l))
    log_all_df <- rbind(log_all_df, 
                        data.frame(dataset = 'sim_rev_1', size = N, lambda = l, 
...
ggsave('figures/Fig_S1_rev.pdf', width=6, height=5, units='in')
