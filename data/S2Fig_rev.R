...
for(N in c(50, 100, 200, 500)){
  for(l in c((0:4)/50, (2:20)/20)){
    n_iter <- nrow(read.csv(sprintf('result/ScalingStudy/sim_rev_1-N%d-fmds-%.2f-log.csv', N, l)))
    iter_df <- rbind(iter_df, data.frame(dataset='sim_rev_1', size=N, lambda=l, iteration=n_iter))
  }
}
...
ggplot(data=iter_df) +
  geom_point(aes(x=lambda, y=iteration-1, shape=as.factor(size))) +
  # geom_errorbar(aes(x=lambda, ymin=iter_mean-1-iter_std, ymax=iter_mean-1+iter_std),
  #               width=0.03, color='black', size=0.25) +
  geom_line(aes(x=lambda, y=iteration-1, linetype = as.factor(size)), size=0.25) +
...
ggsave('figures/Fig_S2_rev.pdf', width=5, height=4, units='in')