...
for(k in c(7,75)){ # local, global
  for(m in method_v){
    if(m=='mds'){
      z_embed <- read.csv('result/Evaluation/sim_1-mds-Z.csv')
      
      res <- rp_eval2(dm=as.matrix(dist(sim_data)), y_orig=y_sim, z_emb=z_embed)
      c <- calcContinuityFromDist(distReference=dist(sim_data), distLowDim=dist(z_embed), kTM=k)
      t <- calcTrustworthinessFromDist(distReference = dist(sim_data), distLowDim = dist(z_embed), kTM = k)
...
# Panel: Continuity v. Trustworthiness, k = 7
ggplot(data=eval_df[eval_df$k==7,]) +
...
ggsave('figures/fig4A_v3.pdf', width=2.55, height=2, units='in')


# Panel: Continuity v. Trustworthiness, k = 75
ggplot(data=eval_df[eval_df$k==75,]) +
  geom_point(aes(x=continuity, y=trustworthiness, shape=method, color=method, 
                 fill=as.factor(hyperparameter)), size=2.5, stroke=0.5) +
  scale_shape_manual(values=c(21,22,4,23,24,25)) + 
  # ('fmds', 'iso', 'mds', 'tsne', 'umap_s', 'umap_u')
...
ggsave('figures/fig4B_v3.pdf', width=2.55, height=2, units='in')
...
ggsave('figures/fig4C_v2.pdf', width=2.1, height=2, units='in')
...
ggsave('figures/fig4D_v2.pdf', width=3, height=2, units='in')