...
method_v <- c('fmds','mds', 'umap_s', 'umap_u', 'tsne', 'iso', 'nn')
...
for(k in c(3,27)){ # local, global
  for(m in method_v){
    ...
    params_v <- if(m=='fmds') params1_v else if (m=='umap_s' | m=='umap_u') params2_v else params3_v
    for(p in params_v){
      z_embed <- 
        if(m=='fmds'){
...
eval_df$hyperparameter[(eval_df$method=='tsne'|eval_df$method=='iso')&eval_df$hyperparameter==10] <- 11
...
ggplot(data=eval_df[eval_df$k==3,]) +
  ...
  scale_shape_manual(values=c(21,22,4,8,23,24,25)) + 
  # ('fmds', 'iso', 'mds', 'nn', 'tsne', 'umap_s', 'umap_u')
  scale_color_manual(values=c('red','grey30','grey30','grey30','grey30','grey30',
                              'grey30','grey30')) +
  scale_fill_manual(values=c('white','grey99', 'grey75', 'grey25',
                             'grey99', 'grey75', 'grey75', 'grey25', 'grey25')) +
  # 0, 0.2, 0.4, 0.8, 5, 7, 10, 11, 30 (11 for tsne/iso)
...
ggsave('figures/fig5A_v3.pdf', width=2.55, height=2, units='in')
...
ggsave('figures/fig5B_v3.pdf', width=2.55, height=2, units='in')
...
ggsave('figures/fig5C_v2.pdf', width=2.1, height=2, units='in')
...
ggsave('figures/fig5D_v2.pdf', width=3, height=2, units='in')
